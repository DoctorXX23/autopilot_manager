...
DBusInterface::DBusInterface(HandlerFunction handler): handler_(handler) {
	...
	if (!dbus_connection_register_object_path(dbus_connection_, OBJECT_NAME, &server_vtable, this)) {
		std::cerr << std::endl << "[DBusInterface] dbus_connection_register_object_path: " << dbus_error.message << std::endl;
...
void DBusInterface::Register() {
	...
	while(true) {
		const int return_code = dbus_bus_request_name(dbus_connection_, DBusInterface::BUS_NAME, 0, &dbus_error);
		...
		if (dbus_error_is_set(&dbus_error)) {
			std::cerr << std::endl << "[DBusInterface] dbus_bus_request_name: " << dbus_error.message << std::endl;
			dbus_error_free(&dbus_error);
...
DBusHandlerResult DBusInterface::MessageHandler(DBusConnection *conn, DBusMessage *message, void *data) {
	std::cout << "[DBusInterface] Received message: " << dbus_message_get_interface(message) << " " << dbus_message_get_member(message)  << " " << dbus_message_get_path(message) << std::endl;
	...
	if (dbus_message_is_method_call(message, "org.freedesktop.DBus.Introspectable", "Introspect")) {
		if (!(reply = dbus_message_new_method_return(message))) {
			...
			dbus_message_append_args(reply,
					 DBUS_TYPE_STRING, &interface->introspection,
					 DBUS_TYPE_INVALID);
		}
