...
bool AutopilotManagerConfig::InitFromMessage(DBusMessage *request ) {
    if(request != nullptr) {
        ...
        dbus_message_get_args(request, &error,
                                DBUS_TYPE_BOOLEAN, &returnedEnabled,
                                DBUS_TYPE_STRING, &returnedDecisionMakerType,
                                DBUS_TYPE_DOUBLE, &returnedCollAvoidThr,
                                DBUS_TYPE_STRING, &returnedCollAvoidOnCondTrue,
                                DBUS_TYPE_STRING, &returnedCollAvoidOnCondFalse,
                                DBUS_TYPE_INVALID);
        if (dbus_error_is_set(&error)) {
            std::cerr << "[Autopilot Manager Config] Failed getting message arguments: " << error.message << std::endl;
            ...
            std::cout << "    simple_collision_avoid_distance_threshold: " << simple_collision_avoid_distance_threshold << std::endl;
            std::cout << "    simple_collision_avoid_distance_on_condition_true: " << simple_collision_avoid_distance_on_condition_true << std::endl;
            std::cout << "    simple_collision_avoid_distance_on_condition_false: " << simple_collision_avoid_distance_on_condition_false << std::endl;
            std::cout << "]" << std::endl;
            return true;
        }
    }
...
bool AutopilotManagerConfig::AppendToMessage(DBusMessage *reply ) const {
    if(reply != nullptr) {
        dbus_message_append_args(reply,
                DBUS_TYPE_BOOLEAN, &autopilot_manager_enabled,
...
bool AutopilotManagerConfig::WriteToFile(const std::string &config_path) const {
    ...
    if (file.is_open()) {
        ...
        file << "simple_collision_avoid_distance_threshold=" << std::to_string(simple_collision_avoid_distance_threshold) << std::endl;
        file << "simple_collision_avoid_distance_on_condition_true=" << simple_collision_avoid_distance_on_condition_true << std::endl;
        file << "simple_collision_avoid_distance_on_condition_false=" << simple_collision_avoid_distance_on_condition_false << std::endl;
        file.close();
    ...
    if (file.is_open()) {
        ...
        while (std::getline(file, line)) {
            ...
            if (line.find("autopilot_manager_enabled") != std::string::npos) {
                ...
                autopilot_manager_enabled = static_cast<bool>(std::stoi(sin.str()));
            }
            ...
            if (line.find("simple_collision_avoid_distance_on_condition_true") != std::string::npos) {
                std::cout << "simple_collision_avoid_distance_on_condition_true: " << sin.str() << std::endl;
            ...
            if (line.find("simple_collision_avoid_distance_on_condition_false") != std::string::npos) {
                std::cout << "simple_collision_avoid_distance_on_condition_false: " << sin.str() << std::endl;
